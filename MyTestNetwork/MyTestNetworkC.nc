...
implementation {
  ...
  void sendMessage() {
    MyTestNetworkMsg* msg = (MyTestNetworkMsg*)call Send.getPayload(&packet, sizeof(MyTestNetworkMsg));
    uint16_t metric;
    am_addr_t parent;
    ...
    if (call Send.send(&packet, sizeof(MyTestNetworkMsg)) != SUCCESS) {
      failedSend();
      call Leds.led0On();
      dbg("MyTestNetworkC", "%s: Transmission failed.\n", __FUNCTION__);
    }
    else {
      sendBusy = TRUE;
      seqno++; 
      dbg("MyTestNetworkC", "%s: Transmission succeeded.\n", __FUNCTION__);
    }
  }
  ...
  event void Timer.fired() {
    ...
    dbg("MyTestNetworkC", "MyTestNetworkC: Timer fired.\n");
  ...
  event void Send.sendDone(message_t* m, error_t err) {
    ...
    sendBusy = FALSE;
    dbg("MyTestNetworkC", "Send completed.\n");
  }
  ...
  Receive.receive(message_t* msg, void* payload, uint8_t len) {
    dbg("MyTestNetworkC", "Received packet at %s from node %hhu.\n", sim_time_string(), call CollectionPacket.getOrigin(msg));
    call Leds.led1Toggle();    
    if (!call Pool.size() <= (MY_TEST_NETWORK_QUEUE_SIZE < 4)? 1:3)  {
